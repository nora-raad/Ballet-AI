...
def get_point(kps, idx, min_conf=0.1):  # Lowered from 0.3
...
def extract_features_from_video(json_files):
    ...
    for json_file in sorted(json_files):
        ...
        # Get points (lower conf for legs to 0.1)
        points = {name: get_point(merged_kps, idx, 0.1) for name, idx in BODY_25_INDICES.items()}

        # Filter: Require key legs (relaxed)
        if None in (points['LKnee'], points['LAnkle'], points['RKnee'], points['RAnkle']):
            continue

        # Foot points with lower conf (0.1)
        foot_points = [p for idx in FOOT_INDICES if (p := get_point(merged_kps, idx, 0.1))]

        if len(foot_points) < 2:  # Relaxed from 4
            continue
...
# MediaPipe setup and mapping for camera mode
mp_pose = mp.solutions.pose
MP_INDICES = {
    'LHip': 23, 'RHip': 24,
    'LKnee': 25, 'RKnee': 26,
    'LAnkle': 27, 'RAnkle': 28,
    'LHeel': 29, 'RHeel': 30,  # Approximate
    'LBigToe': 31, 'RBigToe': 32,  # Approximate
    'LSmallToe': 31, 'RSmallToe': 32  # Duplicate big toe as approx
}
...
def extract_features_from_mediapipe(img):
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        for name, idx in MP_INDICES.items():
            lm = landmarks[idx]
            if lm.visibility > 0.2:  # Approx your min_conf
        ...
        if None in (points['LKnee'], points['LAnkle'], points['RKnee'], points['RAnkle']) or len(foot_points) < 4:
            return None
        
        # Compute features (copy your logic from extract_features_from_video, using points)
        lankle, rankle = points['LAnkle'], points['RAnkle']
        lknee, rknee = points['LKnee'], points['RKnee']
        ...
        if avg_leg_len > 0:
            features['ankle_dist'] /= avg_leg_len
            features['foot_spread'] /= avg_leg_len
        return features  # For camera, single-frame features (no average)
